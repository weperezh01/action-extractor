...
export async function GET(req: NextRequest) {
  try {
    ...
    const items = await listCommunityHomePostsForUser({
      userId: user.id,
      limit,
    })
    ...
    const message = error instanceof Error ? error.message : 'No se pudo cargar el feed Home.'
    console.error('[ActionExtractor] community feed home GET error:', message)
    return NextResponse.json({ error: 'No se pudo cargar el feed Home.' }, { status: 500 })
  }
}
