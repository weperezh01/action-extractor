    ...
    <main className="min-h-screen bg-slate-50 text-slate-900 p-6 md:p-10">
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-wider font-semibold text-slate-500">Panel Admin</p>
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white p-4">
          ...
          <button
            ...
            className="h-10 px-4 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 disabled:bg-slate-400"
          >
          ...
          <>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <p className="text-xs text-slate-500 uppercase">Usuarios totales</p>
                <p className="text-3xl font-bold mt-1">{formatNumber(payload.stats.total_users)}</p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <p className="text-xs text-slate-500 uppercase">Extracciones totales</p>
                <p className="text-3xl font-bold mt-1">{formatNumber(payload.stats.total_extractions)}</p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <p className="text-xs text-slate-500 uppercase">Usuarios activos (7d)</p>
                <p className="text-3xl font-bold mt-1">{formatNumber(payload.stats.active_users_7d)}</p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <p className="text-xs text-slate-500 uppercase">Extracciones (24h)</p>
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                ...
                <div className="space-y-3 text-sm">
                  <div>
                    <p className="text-slate-500">Videos únicos ({payload.estimation.periodDays}d)</p>
                    <p className="text-xl font-bold">{formatNumber(payload.estimation.estimatedClaudeCalls)}</p>
                  </div>
                  <div>
                    <p className="text-slate-500">Costo unitario estimado</p>
                    <p className="font-semibold">
                      {formatCurrencyUsd(payload.estimation.estimatedCostPerExtractionUsd)}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-500">Costo total estimado</p>
                    <p className="text-2xl font-bold text-indigo-700">
                      {formatCurrencyUsd(payload.estimation.estimatedClaudeCostUsd)}
                    </p>
                  </div>
                  <p className="text-xs text-slate-500">{payload.estimation.note}</p>
                </div>
              </div>
            </div>
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                ...
                <div className="space-y-3">
                  {payload.stats.top_videos.length === 0 ? (
                    <p className="text-sm text-slate-500">No hay videos para mostrar.</p>
                  ) : (
                    payload.stats.top_videos.map((item) => (
                      <div
                        key={item.video_id}
                        className="grid grid-cols-[64px_1fr_auto] gap-3 items-center border border-slate-100 rounded-lg p-2"
                      >
                        {item.thumbnail_url ? (
                          <img
                          ...
                          <div className="w-16 h-10 rounded bg-slate-100 border border-slate-200" />
                        )}
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-slate-800 truncate">
                            {item.video_title || item.video_id}
                          </p>
                          <p className="text-xs text-slate-500 truncate">{item.video_id}</p>
                        </div>
                        <p className="text-sm font-semibold text-slate-700">{formatNumber(item.total)}</p>
                      </div>
                    ))
                  )}
                </div>
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                ...
                <div className="space-y-3">
                  {payload.stats.extraction_modes.length === 0 ? (
                    <p className="text-sm text-slate-500">No hay datos de modos en este periodo.</p>
                  ) : (
                    payload.stats.extraction_modes.map((item) => (
                      ...
                      >
                        <p className="text-sm text-slate-700">{resolveModeLabel(item.extraction_mode)}</p>
                        <p className="text-sm font-semibold text-slate-900">{formatNumber(item.total)}</p>
                      </div>
                    ))
                  )}
                </div>
